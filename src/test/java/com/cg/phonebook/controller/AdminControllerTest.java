...
public class AdminControllerTest {
	...
	void test2_findByUserId() throws Exception {
		...
		when(adminService.findUserByUserIdentifier(Mockito.any())).thenReturn(user);
	...
	void test3_findByUserId() throws Exception {
		...
		Mockito.when(adminService.findUserByUserIdentifier(Mockito.any())).thenReturn(null);
		this.mockMvc.perform(get("/api/phonebook/byUserIdentifier/US01"))
		.andExpect(status().isBadRequest());
	}
	
	@Test
	void test4_findByPhoneNumber() throws Exception {
		
		UserEntity user1 = new UserEntity("Sai","krishna","US01","saikrishna@gmail.com","9923456781","Chaitanya@1","Chaitanya@1","address",null);
		Mockito.when(adminService.searchByPhoneNumber(Mockito.any())).thenReturn(user1);
		this.mockMvc.perform(get("/api/phonebook/byUserphoneNumber/9923456781"))
		.andExpect(status().isOk());
	}
	
	@Test
	void test5_findByPhoneNumber() throws Exception {
		
		Mockito.when(adminService.searchByPhoneNumber(Mockito.any())).thenReturn(null);
		this.mockMvc.perform(get("/api/phonebook/byUserphoneNumber/9923456781"))
		.andExpect(status().isBadRequest());
	}
	...
	void test6_deleteByUserId() throws Exception{
		...
		user.setPassword("Chaitanya@1");
		user.setConfirmPassword("Chaitanya@1");
		when(adminService.deleteUserByUserIdentifier(Mockito.any())).thenReturn(true);
	...
	void test7_deleteByUserId() throws Exception{
		...
		Mockito.when(adminService.deleteUserByUserIdentifier(Mockito.any())).thenReturn(false);
		this.mockMvc.perform(delete("/api/phonebook/byIdentifier/US01"))
		.andExpect(status().isBadRequest());
	...
	@Test
	void test8_deleteByPhoneNumber() throws Exception {
		...
		user.setPassword("Chaitanya@1");
		user.setConfirmPassword("Chaitanya@1");
		Mockito.when(adminService.removeUserByPhoneNumber(Mockito.any())).thenReturn(true);
		this.mockMvc.perform(delete("/api/phonebook/9923456781"))
		.andExpect(status().isOk());
		
	}
	
	@Test
	void test9_deleteByPhoneNumber() throws Exception {
		
		Mockito.when(adminService.removeUserByPhoneNumber(Mockito.any())).thenReturn(false);
		this.mockMvc.perform(delete("/api/phonebook/9923456781"))
		.andExpect(status().isBadRequest());
